...
import { cartSelector, clearProducts } from "../../redux/cartSlice";
...
const Cart: React.FC = () => {
	...
	const totalCount = items.reduce((sum: number, item: PizzaType) => sum + item.count, 0)
	const onClickClear = () => {
		if (items.length > 0 && window.confirm('Are you sure you want to remove?')) {
			dispatch(clearProducts())
		}
	}
	const pizzas = items.map((item: PizzaType) => {
